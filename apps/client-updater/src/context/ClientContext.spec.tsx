...
describe('ClientContext', () => {
  ...
  it('should throw error if used outside provider', () => {
    ...
    console.error = vi.fn();
    expect(() => renderHook(() => useClientContext())).toThrow(
  ...
  it('should set and reset client state correctly', () => {
    ...
    const mockClient: ClientProfile = {
      clientId: '1',
      clientContext: '',
      clientNameDetails: { firstName: 'Test', lastName: '', middleName: '' },
      emails: [{ emailAddress: '', emailType: '', emailStatusCode: '' }],
      telephone: [
        { countryCode: '', areaCode: '', nonNANNumber: '', phoneType: '' },
      ],
      postalAddress: [
        {
          addressLine1: '',
  ...
  it('should handle wizard flow state (selectSection, pendingUpdate, reset)', () => {
    const { result } = renderHook(() => useClientContext(), { wrapper });
    ...
    act(() => {
      result.current.selectSection('name');
    });
    expect(result.current.selectedSection).toBe('name');

    act(() => {
      result.current.setPendingUpdate({
        section: 'name',
        oldValue: { firstName: 'Test', lastName: '', middleName: '' },
        newValue: { firstName: 'New Test', lastName: '', middleName: '' },
      });
    });
